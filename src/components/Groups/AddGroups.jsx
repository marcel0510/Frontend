...
export default function AddGroups() {
  ...
  const [calendar, isEdit, setIsEdit, setIsSee] = useOutletContext(); //Informacion del padre
  ...
  const {
    data: calendarData,
    isLoading: loadCalendar,
    isError: errorCalendar,
  ...
  const isLoading = loadSubjects || loadClassrooms || loadCalendar;
  const isError = errorSubjects || errorClassrooms || errorCalendar;

  useEffect(() => {
    if (!isLoading && isInitialMount.current) {
      setSubjects(subjectsData);
      setClassrooms(classroomsData);
      setCurrentCalendar(calendarData.period);
      setForm({ ...form, calendarId: calendarData.id });
      isInitialMount.current = false;
    }
    setIsSee(false);
    setIsEdit(false);
    filterData();
  }, [filters, isLoading, subjectsData, classroomsData, calendarData]);
